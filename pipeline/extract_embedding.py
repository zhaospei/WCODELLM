...
def main():
    ...
    output_dir = args.generate_dir.replace('temp', 'embedding')
    ...
    for layer in args.layers:
        ...
        for example in tqdm.tqdm(dataset, total=len(dataset)):
            ...
            for j in range(len(task_generation_seqs['generations'])):
                ...
                try:
                    last_token_code_embedding = layer_embedding
            ...
            print(len(results))
        print('dumps to pkl file ', len(results))
        outfile = os.path.join(output_dir, f'LFCLF_embedding_{args.dataset}_{model_name}_{layer}.pkl')
